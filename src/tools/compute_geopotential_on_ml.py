#!/usr/bin/env python3
"""
Imported from https://confluence.ecmwf.int/plugins/viewsource/viewpagesrc.action?pageId=158636068
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("-l", "--levelist", help="levelist to store", default="all")
    parser.add_argument(
        "-o", "--output", help="name of the output file", default="z_out.grib"
    )
    parser.add_argument(
        "t_q",
        metavar="tq.grib",
    ...
    if args.levelist == "all":
        ...
        args.levelist = [int(lev) for lev in args.levelist.split("/")]
    return args
...
def main() -> None:
    ...
    print(
        "Arguments: %s" % ", ".join(["%s: %s" % (k, v) for k, v in vars(args).items()])
    )
...
def get_initial_values(idx: argparse.Namespace, keep_sample: bool = False) -> dict:
...
def check_max_level(idx: argparse.Namespace, values: dict) -> None:
...
def get_surface_pressure(idx: argparse.Namespace) -> float:
...
def get_ph_levs(values: dict, level: int) -> tuple[float, float]:
...
def compute_z_level(
    idx: argparse.Namespace, lev: int, values: dict, z_h: float
) -> tuple[float, float]:
    ...
    t_level = t_level * (1.0 + 0.609133 * q_level)
...
def production_step(idx: argparse.Namespace, step: int, values: dict, fout) -> None:
    """Compute z at half & full level for the given level, based on t/q/sp"""
