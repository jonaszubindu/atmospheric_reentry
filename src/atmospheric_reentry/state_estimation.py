...
class StateEstimation:
    ...
    def __init__(self) -> None:
        ...
        self.tstep = 0  # always start at t zero for now
        self.state_cartesian = np.zeros(6)  # [3 position 3 velocity]
        self.state_geodetic = np.zeros(6)  # [3 position 3 velocity]

    def estimate_state(self, tstep: int) -> None:
        """Simple state estimation function that could be expanded to
        include sensor noise and filtering.

        For now, just update the time step, but this could be
    ...
    def get_position_cartesian(self) -> np.ndarray:
        """Get the current position of the rocket."""
        return self.state_cartesian[:3]

    def get_velocity_cartesian(self) -> np.ndarray:
        """Get the current velocity of the rocket."""
        return self.state_cartesian[3:]

    def set_position_cartesian(self, new_position: np.ndarray) -> None:
        """Set the current position of the rocket."""
        self.state_cartesian[:3] = new_position

    def set_velocity_cartesian(self, new_velocity: np.ndarray) -> None:
        """Set the current velocity of the rocket."""
        self.state_cartesian[3:] = new_velocity

    def get_position_geodetic(self) -> np.ndarray:
        """Get the current position of the rocket in geodetic coordinates."""
        self._cartesian_to_geodetic()  # lazily update geodetic position from cartesian position
        return self.state_geodetic[:3]

    def get_velocity_geodetic(self) -> np.ndarray:
        """Get the current velocity of the rocket in geodetic coordinates."""
        self._cartesian_to_geodetic_velocity()  # lazily update geodetic velocity from cartesian velocity
        return self.state_geodetic[3:]

    def _set_position_geodetic(self, new_position: np.ndarray) -> None:
        """Set the current position of the rocket in geodetic coordinates."""
        self.state_geodetic[:3] = new_position

    def _set_velocity_geodetic(self, new_velocity: np.ndarray) -> None:
        """Set the current velocity of the rocket in geodetic coordinates."""
        self.state_geodetic[3:] = new_velocity
    ...
    def _cartesian_to_geodetic(self) -> None:
        ...
        self._set_position_geodetic(
            np.array(pm.ecef2geodetic(position[0], position[1], position[2]))
        )
    ...
    def _cartesian_to_geodetic_velocity(self) -> None:
        ...
        lat, lon, _ = position
        self._set_velocity_geodetic(
            np.array(pm.ecef2enuv(velocity[0], velocity[1], velocity[2], lat, lon))
        )
    ...
    def convert_velocity_geodetic_to_cartesian(
        velocity_geodetic: np.ndarray, lat: float, lon: float
    ) -> np.ndarray:
        """Convert geodetic velocity (v_lat, v_lon, v_alt) to cartesian
        velocity (vx, vy, vz). Assumes velocity is in ENU (East, North, Up)
        coordinates for geodetic velocity and we want to convert it to ECEF
    ...
    @staticmethod
    def convert_geodetic_to_cartesian(position_geodetic: np.ndarray) -> np.ndarray:
        """Convert geodetic coordinates (latitude, longitude, altitude) to cartesian
        coordinates (x, y, z)."""
        lat, lon, alt = position_geodetic
        return np.array(pm.geodetic2ecef(lat, lon, alt))
